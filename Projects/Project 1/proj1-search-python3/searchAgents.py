...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.checkSuccess = {self.corners[0]: False,
                             self.corners[1]: False,
                             self.corners[2]: False,
                             self.corners[3]: False}
        if self.startingPosition in self.corners:
            self.checkSuccess[self.startingPosition] = True
    ...
    def getStartState(self):
        ...
        space)
        """
        return (self.startingPosition, self.checkSuccess)
    ...
    def isGoalState(self, state):
        ...
        if state[0] in self.corners:
            ...
            if state[1][self.corners[0]] and state[1][self.corners[1]] and state[1][self.corners[2]] and state[1][self.corners[3]]:
                return True
        return False
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                succ_state = (nextx, nexty)
                temp = dict(state[1])
                if succ_state in self.corners:
                    temp[succ_state] = True
                successors.append(((succ_state, temp), action, 1))
...
def manhattanDistance(position, foodItem, info={}):
    xy1 = position
    xy2 = foodItem
    return abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1])


def euclideanDistance(position, foodItem, info={}):
...
def cornersHeuristic(state, problem):
    ...
    if position in corners:
        return 0
    for coor, check in state[1].items():

        if not check:
            needed_corners.append(coor)
    temp_shortest = sys.maxsize
    print("Needed food", needed_corners)
    for ele in needed_corners:
        num_walls = 0
        x, y = ele
        if(x+1, y) in problem.walls.asList():
            num_walls = num_walls + 1
        if(x, y+1) in problem.walls.asList():
            num_walls = num_walls + 1
        if(x-1, y) in problem.walls.asList():
            num_walls = num_walls + 1
        if(x, y-1) in problem.walls.asList():
        ...
        temp_shortest = min(temp_shortest,
                            manhattanDistance(position, ele))
...
def foodHeuristic(state, problem):
    ...
    position = state[0]
    needed_food = state[1].asList()

    temp_long = 0
    for food in needed_food:

        temp_long = max(temp_long,
